...
def index(request: Request):
    ...
    scroll_id = request.query_params.get("scroll")
...
def carrinho_page(request: Request):
    ...
    avatar = buscar_avatar(request.cookies.get("usuario_id"))
...
def adicionar_carrinho(request: Request, id_filme: int = Form(...)):
    ...
    response = RedirectResponse(f"/?scroll={id_filme}", status_code=303)
...
def locacoes_page(request: Request):
    ...
    locacoes = [dict(l) for l in cursor.fetchall()]
    conn.close()
    avatar = buscar_avatar(usuario_id)
...
def buscar_avatar(usuario_id):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT avatar, nome FROM Usuario WHERE id_usuario=%s", (usuario_id,))
    usuario = cursor.fetchone()
    conn.close()
    if usuario and usuario["avatar"]:
